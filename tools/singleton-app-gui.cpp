...
SingletonApp::SingletonApp(int &argc, char *argv[], const char *appName, bool allowSecondary, Options options, int timeout)
        : QApplication(argc, argv), mIsSync(false)
{
    bool            ret = false;
...
{
    mParser.setApplicationDescription (tr("Andsec沙盒文件转移工具"));
    mParser.addHelpOption();
    mParser.addVersionOption();

    QCommandLineOption s(QStringList()     << "s"   << "sync", QCoreApplication::translate ("main-app-c", "文件同步"));
    mParser.addOption (s);

    mParser.process (*this);

    mIsSync = mParser.isSet (s);

    // auto l = mParser.positionalArguments();
    // for (auto& f : l) {
    //     if (0 == access(f.toUtf8().constData(), F_OK)) {
    //         char* ff = utils_file_path_normalization (f.toUtf8().constData());
    //         qDebug() << "file: " << f << " ==> " << ff;
    //         if (ff) { mOptionFiles << ff;} else {qWarning() << "file: " << f << " normalization error!";}
    //     }
    //     else {
    //         qWarning() << "file '" << f << "' is not exists!";
    //     }
    // }

    qDebug() << "is sync: " << mIsSync;
}
...
bool SingletonApp::isSync() const
{
    return mIsSync;
}
