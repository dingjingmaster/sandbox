...
#ifndef sandbox_LOG_FILE_H
#define sandbox_LOG_FILE_H
...
typedef struct
{
    ...
    /*  0*/
    FS_RECORD_TYPES magic; /* The magic is "RSTR". */
    /*  4*/
...
typedef struct
{
    ...
    le32 client_name_length; /* Length of client name in bytes.  Should
                                   always be 8. */
    /* 32*/
    FSChar client_name[64]; /* Name of the client in Unicode.  Should
...
{
    /*  0   NTFS_RECORD; -- Unfolded here as gcc doesn't like unnamed structs. */
    FS_RECORD_TYPES magic; /* Usually the magic is "RCRD". */
    le16 usa_ofs; /* See NTFS_RECORD definition in layout.h.
                                   When creating, set this to be immediately
                                   after this header structure (without any
                                   alignment). */
...
typedef enum
{
    ...
    /* This has nothing to do with the log record. It is only so gcc knows to make the flags 16-bit. */
...
extern bool fs_check_logfile(FSAttr * log_na, RESTART_PAGE_HEADER ** rp);
extern bool fs_is_logfile_clean(FSAttr * log_na, RESTART_PAGE_HEADER * rp);
extern int fs_empty_logfile(FSAttr * na);



C_END_EXTERN_C

#endif // sandbox_LOG_FILE_H
