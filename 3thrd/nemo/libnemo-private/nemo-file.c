                ...
                gboolean custom)
{
    ...
    if (g_strcmp0 (file->details->edit_name, edit_name) != 0) {
        ...
        if (g_strcmp0 (file->details->display_name, edit_name) == 0) {
            file->details->edit_name = g_strdup(file->details->display_name);
        } else {
            file->details->edit_name = g_strdup(edit_name);
        }
    }
              ...
              gboolean update_name)
{
    ...
    if (g_strcmp0 (file->details->owner, owner) != 0) {
        changed = TRUE;
        g_clear_pointer (&file->details->owner, g_free);
        file->details->owner = g_strdup (owner);
    }

    if (g_strcmp0 (file->details->owner_real, owner_real) != 0) {
        changed = TRUE;
        g_clear_pointer (&file->details->owner_real, g_free);
        file->details->owner_real = g_strdup (owner_real);
    }

    if (g_strcmp0 (file->details->group, group) != 0) {
        changed = TRUE;
        g_clear_pointer (&file->details->group, g_free);
        file->details->group = g_strdup (group);
    }
    ...
    if (g_strcmp0 (file->details->filesystem_id, filesystem_id) != 0) {
        changed = TRUE;
        g_clear_pointer (&file->details->filesystem_id, g_free);
        file->details->filesystem_id = g_strdup(filesystem_id);
    }
    ...
    if (update_name) {
        ...
        if (file->details->name == NULL ||
            ...
            if (g_strcmp0 (file->details->display_name, name) == 0) {
                file->details->name = g_strdup (file->details->display_name);
            } else {
