...
static int ntfs_bitmap_set_bits_in_run(ntfs_attr *na, s64 start_bit, s64 count, int value)
{
    ...
    if (!na || start_bit < 0 || count < 0) {
        errno = EINVAL;
        ntfs_log_perror("%s: Invalid argument (%p, %lld, %lld)", __FUNCTION__, na, (long long)start_bit, (long long)count);
        return -1;
    }
    ...
    do {
        ...
        if (count > 0 && bit) {
            ...
            if (!lastbyte_pos) {
                // FIXME: Eeek! BUG!
                ntfs_log_error("Lastbyte is zero. Leaving inconsistent metadata.\n");
                errno = EIO;
                goto free_err_out;
            }
            ...
            if (lastbyte_pos <= bufsize) {
                ...
                if (br != 1) {
                    ...
                    ntfs_log_perror("Reading of last byte failed (%lld). Leaving inconsistent metadata", (long long)br);
                    goto free_err_out;
                }
        ...
        if (br != bufsize) {
            ...
            ntfs_log_perror("Failed to write buffer to bitmap (%lld != %lld). Leaving inconsistent metadata", (long long)br, (long long)bufsize);
            goto free_err_out;
        }
        ...
        if (lastbyte && count != 0) {
            // FIXME: Eeek! BUG!
            ntfs_log_error("Last buffer but count is not zero (%lld). Leaving inconsistent metadata.\n", (long long)count);
            errno = EIO;
            goto free_err_out;
        }
    } while (count > 0);
